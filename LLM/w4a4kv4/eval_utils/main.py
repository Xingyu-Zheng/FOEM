...
def ptq_model(args, model, model_args=None):
    ...
    if args.enable_aq_calibration:
        add_input_quantization(model, args)
    ...
    if args.w_bits < 16:
        ...
        end_time = time.time()
        quant_time = end_time - begin_time
        print(f'time cost: {quant_time}s')
        with open(f"quant_time_logs/time.txt", "w") as f:
            f.write(f'time cost: {quant_time}s')
