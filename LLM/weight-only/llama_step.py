...
def llama_sequential(model, dataloader, dev, fp_path):
    # if args.gptqv2:
    #     model_fp = LlamaForCausalLM.from_pretrained(fp_path, torch_dtype='auto', device_map="cpu")
    #     model_fp.seqlen = 2048
    #     # model_fp = model_fp.cuda()
    #     # model_fp = model_fp.cpu()
    #     model_fp.eval()
    # import pdb; pdb.set_trace()
    # model_fp.model.layers
    print('Starting ...')
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            if args.gptqv2:
                def add_batch_v2(name):

                    def tmp(_, inp, out):
                        gptq[name].add_batch_v2(inp[0].data, out.data)

                    return tmp
                handles = []
                for name in subset:
                    gptq[name].nsamples = 0
                    handles.append(subset[name].register_forward_hook(add_batch_v2(name)))
                for j in range(args.nsamples):
                    # print(inps[j]-inps_[j])
                    outs_[j] = layer(inps_[j].unsqueeze(0), attention_mask=attention_mask, position_ids=position_ids)[0]
                for h in handles:
                    h.remove()

            for name in subset:
                scale, zero, g_idx, error = gptq[name].fasterquant(percdamp=args.percdamp, groupsize=args.groupsize, actorder=args.act_order, name=name, ours=args.ours, ours_v2=args.ours_v2, alpha=args.alpha, beta=args.beta, gptqv2=args.gptqv2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gptqv2', action='store_true', help='')
    ...
    if args.eval:
        ...
        from evaluate_ import eval_ours
        model = model.cuda()
